...
class AutoEncoder(nn.Module):
    ...
    def sample(self, num_samples, t, eps_list=None):
        scale_ind = 0
        ...
        if eps_list is None:
            ...
            z = dist.sample_given_eps(eps_list[eps_idx])
            eps_idx += 1

        z_list.append(z)
        log_p_list.append(dist.log_p(z))
        ...
        log_p_total = 0.
        for log_p_conv in log_p_list:
            log_p_zi = torch.sum(log_p_conv, dim=[1, 2, 3])

            log_p_total += log_p_zi
        ...
        logits = self.image_conditional(s)
        return logits, z_list, log_p_total
    ...
    def decoder_output(self, logits):
        if self.dataset == 'mnist':
            return Bernoulli(logits=logits)
                              ...
                              'ffhq',
                              'lsun_bedroom_128', 'lsun_bedroom_256', 'lsun_church_128', 'lsun_church_64'}:
            if self.num_mix_output == 1:
    ...
    def max_norm(self):
        mx = []
        """
        for p in self.all_log_norm:
            mx.append(torch.exp(torch.max(p)))
        """
        for l in self.all_conv_layers:
            mx.append(l.get_spectral_norm())

        return torch.stack(mx)
